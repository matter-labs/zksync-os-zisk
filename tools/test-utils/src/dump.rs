...
//! reference outputs. This module reconstructs the witness the guest needs —
//! storage proofs, account preimages, bytecodes, tree update.
...
/// Sizes of the witness the conversion reconstructed. The caller decides
...
/// A dump converted to the batch the guest executes.
pub struct Conversion {
    pub batch_input: BatchInput,
    pub stats: ConversionStats,
}

/// Whether the witness arms the guest's canonical block-header-hash assertion
/// with the dump's native value. `Skipped` leaves the hash zero, which makes
/// the guest skip the check, so a panic reflects a mid-execution failure only.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum HeaderHashCheck {
    Armed,
    Skipped,
}
...
/// One preimage, keyed by its blake2s hash.
#[derive(serde::Deserialize)]
...
/// A full flat-storage state snapshot.
#[derive(serde::Deserialize)]
...
/// The block environment the native STF ran under.
#[derive(serde::Deserialize)]
pub struct BlockEnvDump {
    pub number: u64,
    pub timestamp: u64,
    pub base_fee: u64,
    pub gas_limit: u64,
...
/// One transaction of the block, as submitted plus its outcome.
#[derive(serde::Deserialize)]
pub struct TxDump {
    pub signed: String,
    pub gas_used: u64,
...
#[derive(serde::Deserialize)]
pub struct StateDumpBundle {
    pub chain_id: u64,
    pub spec_id: u8,
    pub protocol_version_minor: u32,
    pub da_commitment_scheme: u8,
    pub block: BlockEnvDump,
    pub tree_root_before: String,
    pub leaf_count_before: u64,
    pub tree_root_after: String,
    ...
    pub block_hashes_blake_before: String,
    pub previous_block_hashes: Vec<String>,
    pub native_state_before: String,
    pub native_state_after: String,
    pub native_chain_config_hash: String,
    pub native_batch_output_hash: String,
    pub native_batch_public_input: String,
    #[serde(default)]
    pub chain_config_fri: bool,
    ...
    /// this field verbatim.
    #[serde(default)]
    pub chain_config_max_tx_gas_limit: u64,
    ...
    #[serde(default)]
    pub chain_config_pubdata_content: u8,
    ...
    #[serde(default)]
    pub block_hash_ring_head: String,
}
...
pub fn build_batch_input(d: &StateDumpBundle, header_hash_check: HeaderHashCheck) -> Conversion {
    ...
    let block_env = BlockInput {
        ...
        block_header_hash: match header_hash_check {
            HeaderHashCheck::Skipped => B256::ZERO,
            HeaderHashCheck::Armed => hb256(&d.block_header_hash),
        },
        l2_to_l1_logs: vec![],
    ...
    let stats = ConversionStats {
        slot_reads: read_slots.len(),
        account_reads: read_accounts.len(),
        bytecodes: codes.len(),
        tree_updates: 0,
        tree_inserts: 0,
    };
    ...
    let stats = ConversionStats {
        tree_updates: updates.len(),
        tree_inserts: inserts.len(),
        ..stats
    };
...
// ---------------------------------------------------------------------------
// Unit smoke tests (run via `cargo test`)
// ---------------------------------------------------------------------------
...
pub(crate) mod tests {
    ...
    fn synthetic_bundle_parses_with_defaults() {
        let d: StateDumpBundle = serde_json::from_str(&synthetic_bundle_json()).expect("parse");
    ...
    fn empty_block_bundle_builds_and_validates() {
        ...
        assert!(
            crate::native_check::check_against_native(&d, &conversion.batch_input).agrees(),
