...
| [docker/zisk-stack/README.md](docker/zisk-stack/README.md) | Deploying the lane as containers: the stack image, the proving-key volume, and the compose file for one GPU machine. |
...
| `docker/` | The pinned containers of the reproducible guest builds, and `docker/zisk-stack/`: the image that runs the ZiSK lane (coordinator, GPU worker, prover daemon) on a GPU machine. |
...
build: the base image, the cargo-zisk release (v1.3.0-alpha) and the ZiSK
Rust toolchain it installs (zisk-4.0.0, sha256-pinned, with the cargo that
orchestrates the build), the committed `Cargo.lock`, and a fixed `/build`
source path.
...
The ZiSK toolchain is pinned at v1.3.0-alpha. Install it with
`ziskup -v 1.3.0-alpha`.
